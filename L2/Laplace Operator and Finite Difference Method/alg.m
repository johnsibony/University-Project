...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Une deuxiËme faÁon de construire la matrice A en utilisant l'opÈrateur Kron
%N=4;
%I = speye(n-1);
%e = ones(n-1,1);
...
%L'erreur entre les valeurs propres calculÈs et analytiques
L=[];
for k=1:size(Vth,2)
L=[L,norm((Vth(k)-Vdisc(k))/Vth(k))];
endfor
% Graphique des fonctions propres calculÈs
%x=1:10;
%figure 1
%plot(x,V(1),x,V(2))
...
% CrÈation de la matrice B ‡ partir de A
B=A;
...
% Utilisation de la fonction transf qui permet l'Èlimination de la ligne et colonne k
k=0;
for i=1:size(h,2)
j=h(i);
B=transf(B,j-k);
